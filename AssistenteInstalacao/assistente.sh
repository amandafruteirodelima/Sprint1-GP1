...
if [ -f /home/ubuntu/monitoramento.jar ]; then echo 'Existe'; else echo 'Não Existe';fi
...
if [ $? = 'Não Existe' ]
    ...
    cd /home/ubuntu/Sprint1-GP1/MonitoramentoJAR
    
    mv monitoramento.jar /home/ubuntu
    sleep 2
    cd /home/ubuntu
    rm -r Sprint1-GP1 -f
...
#instalação APlicação
     ...
     echo  "$(tput setaf 10)[devBot (^O^)/]:$(tput setaf 7)Deseja executar a aplicação?(s/n)"
     read inst3
    if [ \"$inst3\" == \"s\" ]
    then
         echo  "$(tput setaf 10)[devBot (^O^)/]:$(tput setaf 7) A aplicação será executada!Aguarde um momento!"
         sleep 2
        chmod +x monitoramento.jar
        java -jar monitoramento.jar
         else
        echo  "$(tput setaf 10)[devBot (^O^)/]:$(tput setaf 7) A aplicação não será executada!Até mais Dev!"
